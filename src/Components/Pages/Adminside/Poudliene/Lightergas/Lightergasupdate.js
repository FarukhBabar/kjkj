...
const Lightergasupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8001/api/v1/auth/lightergassingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`http://localhost:8001/api/v1/auth/lightergassingle/${params.id}` , {
        method: "put",
        body :formdata
