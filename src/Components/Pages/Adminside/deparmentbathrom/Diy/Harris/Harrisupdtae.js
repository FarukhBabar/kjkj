...
const Harrisupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8001/api/v1/auth/harrissingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`http://localhost:8001/api/v1/auth/harrissingle/${params.id}` , {
        method: "put",
        body :formdata
