...
const Chamoisupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8001/api/v1/auth/chomissingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`http://localhost:8001/api/v1/auth/chomissingle/${params.id}` , {
        method: "put",
        body :formdata
