...
const Camerasupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8001/api/v1/auth/camerasingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`http://localhost:8001/api/v1/auth/camerasingle/${params.id}` , {
        method: "put",
        body :formdata
